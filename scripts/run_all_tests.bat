...
echo ================================================================
echo.
venv\Scripts\python.exe -m pytest tests/test_config.py -v --tb=short
if %errorlevel%==0 (
...
venv\Scripts\python.exe -m pytest tests/test_mock_services.py::TestMockChromaDB -v --tb=short
if %errorlevel%==0 (
...
venv\Scripts\python.exe -m pytest tests/test_mock_services.py::TestMockCollection -v --tb=short
if %errorlevel%==0 (
...
venv\Scripts\python.exe -m pytest tests/test_mock_services.py::TestMockSentenceTransformer -v --tb=short
if %errorlevel%==0 (
...
venv\Scripts\python.exe -m pytest tests/test_mock_services.py::TestGetMockServices -v --tb=short
if %errorlevel%==0 (
...
echo ================================================================
echo 🗄️ PHASE 4: DATABASE MODEL TESTS
echo ================================================================
echo.
venv\Scripts\python.exe -m pytest tests/test_database.py -v --tb=short
if %errorlevel%==0 (
    echo ✅ Database model tests: PASSED
    set /a passed_tests+=10
) else (
    echo ❌ Database model tests: FAILED
    set /a failed_tests+=10
)
set /a total_tests+=10
echo.

echo ================================================================
echo 💾 PHASE 5: CACHE SYSTEM TESTS (Non-Async)
echo ================================================================
echo.
echo 📦 Testing Cache Manager Initialization...
venv\Scripts\python.exe -m pytest tests/test_cache.py::TestCacheManager::test_generate_cache_key -v --tb=short
if %errorlevel%==0 (
...
echo ================================================================
echo 🤖 PHASE 6: MODEL MANAGER TESTS (Non-Async)
echo ================================================================
echo.
echo 📦 Testing Model Manager Memory Stats...
venv\Scripts\python.exe -m pytest tests/test_model_manager.py::TestModelManagerProduction::test_memory_calculation -v --tb=short
if %errorlevel%==0 (
    echo ✅ Memory calculation: PASSED
    set /a passed_tests+=1
) else (
    echo ❌ Memory calculation: FAILED
    set /a failed_tests+=1
)
set /a total_tests+=1
echo.
...
echo - scripts\run_tests.bat          (Full pytest suite)
echo - scripts\test_endpoints.bat     (API endpoint testing)
echo.
echo Development commands:
echo - scripts\setup_local.bat        (Initial setup)
echo - scripts\run_dev.bat            (Start development server)
echo.
